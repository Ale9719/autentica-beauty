...
interface FilosofiaCollageProps {
  titolo?: string;
  testo?: string;
  immagine1?: string;
  immagine2?: string;
...
export default function FilosofiaCollage({ titolo, testo, immagine1, immagine2 }: FilosofiaCollageProps) {
  return (
    <section className="relative px-6 lg:px-24 py-20 lg:py-32 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center bg-ab-cream overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-125 mx-auto h-137.5 lg:h-162.5 mt-8 lg:mt-0">
        ...
        >
          <Image 
            src={immagine1 || '/placeholder-2.jpg'} 
            alt="Oasi Benessere" 
            fill 
        ...
        >
          <Image 
            src={immagine2 || '/placeholder-3.jpg'} 
      ...
      <div className="relative z-10 space-y-6 md:space-y-8 text-center lg:text-left pb-16 lg:pb-0">
        <span className="inline-block text-ab-gold tracking-[0.3em] uppercase text-[11px] md:text-xs font-semibold border-b border-ab-gold/30 pb-2">
          ...
          <h2 
            className="font-serif text-4xl md:text-5xl lg:text-6xl leading-[1.1] text-ab-tortora-dark"
            dangerouslySetInnerHTML={{ __html: titolo || 'Equilibrio biologico, cura scientifica.' }}
          />
        <p className="text-ab-tortora-dark/80 text-base md:text-lg leading-relaxed font-sans font-light max-w-lg mx-auto lg:mx-0">
          {testo || "Nel nostro spazio l'estetica si spoglia della fretta. Ogni seduta inizia con un'analisi dermo-visiva accurata e si sviluppa in un'atmosfera calma, studiata per far rigenerare i tessuti nel profondo."}
        </p>
        <div className="pt-6">
          <Link href="/servizi" className="group text-ab-tortora-dark uppercase tracking-[0.2em] text-[11px] md:text-xs font-semibold inline-flex items-center gap-4 transition-colors hover:text-ab-gold">
